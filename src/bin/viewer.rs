...
fn default_config() -> Config {
    ...
    Config {
        ...
        // The viewer never evolves anything, so every selection weight here is
        // inert — but this block used to enumerate them all and had drifted
        // badly from config.rs (novelty 0.45 vs 0.60, ood 0.0 vs 0.25,
        // recursion/clip predictors 0.60/0.50 vs the 0.0 every shipped config
        // actually runs). A second source of truth nobody reads is worse than
        // none, so defer to config.rs.
